...
Java_uk_lgl_modmenu_FloatingModMenuService_getFeatureList(JNIEnv *env, jobject activityObject) {
    ...
    const char *features[] = {
            OBFUSCATE("Category_Funções do Jogador"), //1
            OBFUSCATE("Toggle_Funçao 1"), //2
            OBFUSCATE("Toggle_Funçao 2"), //3
            OBFUSCATE("Toggle_Funçao 3"), //4
            OBFUSCATE("Toggle_Funçao 4"), //5
...
Java_uk_lgl_modmenu_Preferences_Changes(JNIEnv *env, jclass clazz, jobject obj,
    ...
    if (feature == 1) {
        ...
        feature2 = boolean;
        if (feature2) {
            MakeToast(env, obj, OBFUSCATE("Ativado"), Toast::LENGTH_SHORT);
            // modify & print bytes
            hexPatches.GodMode.Modify();
        } else {
            MakeToast(env, obj, OBFUSCATE("Desativado"), Toast::LENGTH_SHORT);
            //restore & print bytes
...
void *hack_thread(void *) {
    ...
    hexPatches.GodMode = MemoryPatch::createWithHex(libName,
                                                  string2Offset(OBFUSCATE_KEY("0x012345", '-')),
                                                  OBFUSCATE("00 00 A0 E3 1E FF 2F E1"));
